...
class RoboEyes:
    def __init__(self, screen_updater, width=128, height=64, framerate=30, active=False):
        ...
        # Uniform scale to preserve aspect ratio (use min to fit without distortion)
        self.uniform_scale = min(self.scale_x, self.scale_y)
        self.offset_x = (self.width - self.REF_W * self.uniform_scale) // 2
        self.offset_y = (self.height - self.REF_H * self.uniform_scale) // 2
    ...
    def set_auto_blink(self, active, interval=1, variation=4):
        """Set auto blink."""
    ...
    def update(self):
        ...
        # Re-open eyes
        if self.eyeL_open and self.curr['eyeL_h'] <= 1.1 + self.curr['eyeL_height_offset']:
            self.next['eyeL_h'] = self.default_eye_h

        if self.eyeR_open and self.curr['eyeR_h'] <= 1.1 + self.curr['eyeR_height_offset']:
        ...
        if self.sweat:
            ...
            for drop in self.sweat_drops:
                if drop['y'] <= drop['max_y']:
                    ...
                    drop['h'] = 2.0
                drop['x'] = drop['x_initial'] - (drop['w'] / 2)
                dx = int(self.offset_x + drop['x'] * self.uniform_scale)
