...
class RoboEyesApp(App):
    ...
    def exe_cmd(self, cmd):
        ...
        elif cmd == 'tired': e.set_mood(Mood.TIRED)
        ...
        elif cmd == 'idle': e.is_idle = not e.is_idle; print(f"Idle: {e.is_idle}")
        elif cmd == 'curious': e.is_curious = not e.is_curious
        # Positions
        elif cmd == 'n': e.set_pos(PosDir.N)
        elif cmd == 'ne': e.set_pos(PosDir.NE)
        elif cmd == 'e': e.set_pos(PosDir.E)
        elif cmd == 'se': e.set_pos(PosDir.SE)
        elif cmd == 's': e.set_pos(PosDir.S)
        elif cmd == 'sw': e.set_pos(PosDir.SW)
        elif cmd == 'w': e.set_pos(PosDir.W)
        elif cmd == 'nw': e.set_pos(PosDir.NW)
        elif cmd == 'center': e.set_pos(PosDir.CENTER)
        elif cmd in ['q', 'quit']: sys.exit()
        else: print("Unknown command")

    def _on_key(self, instance, keyboard, keycode, text, modifiers):
        # --- FIX: Ensure keycode is a tuple before accessing index [1] ---
        if not isinstance(keycode, tuple) or len(keycode) < 2:
            # If it's not a valid keycode tuple (e.g., it's just an integer from a broken event),
            # return True to stop further processing and avoid a crash.
            return True
        # -------------------------------------------------------------------
        ...
        # Original logic remains the same below
        if k == 'space': e.blink()
        elif k == '1': e.set_mood(Mood.DEFAULT)
        elif k == '2': e.set_mood(Mood.TIRED)
        elif k == '3': e.set_mood(Mood.ANGRY)
        elif k == '4': e.set_mood(Mood.HAPPY)
        elif k == 'c': e.is_cyclops = not e.is_cyclops
        elif k == 'i': e.is_idle = not e.is_idle
        elif k == 'l': e.trigger_anim('laugh')
        # ... (rest of the conditions for arrows)
        elif k == 'up': e.set_pos(PosDir.N)
        elif k == 'down': e.set_pos(PosDir.S)
        elif k == 'left': e.set_pos(PosDir.W)
        elif k == 'right': e.set_pos(PosDir.E)
